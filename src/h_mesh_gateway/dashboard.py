"""Local management dashboard for gateway state, queue depth, and recent events."""
...
def tail_text_file(path: Path, *, max_lines: int = 200) -> list[str]:
    """Return the last lines of a text file if it exists."""
    if not path.exists():
        return []
...
def render_sparkline(points: list[int], *, width: int = 220, height: int = 56) -> str:
    """Render a small inline SVG queue-depth chart."""
    if not points:
...
def summarize_sensor_payload(payload_json: str) -> tuple[str, str]:
    """Summarize a sensor payload into a compact human-readable row."""
    try:
...
class ManagementRepository:
    """Read-only summary view over one or more gateway state databases."""
    ...
    def recent_logs(self, *, max_lines: int = 120) -> list[dict[str, object]]:
        """Return recent log tails for each available gateway log file."""
...
def render_dashboard_html(snapshot: dict[str, object], logs: list[dict[str, object]]) -> str:
    """Render the management summary as a standalone HTML document."""
...
def build_management_handler(repo: ManagementRepository):
    """Build an HTTP request handler bound to one management repository."""
    ...
    class ManagementHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            """Serve a small set of dashboard and API endpoints."""
...
def run_dashboard_server(
    *,
    ...
    """Run the local dashboard HTTP server until interrupted."""
